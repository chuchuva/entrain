class GlobalSetting
  ...
  def self.load_defaults
    default_provider = FileProvider.from(File.expand_path('../../../config/entrain_defaults.conf', __FILE__))
  ...
  class EnvProvider < BaseProvider
    def lookup(key, default)
      var = ENV["ENTRAIN_" << key.to_s.upcase]
      resolve(var , var.nil? ? default : nil)
    end

    def keys
      ENV.keys.select{|k| k =~ /^ENTRAIN_/}.map{|k| k[10..-1].downcase.to_sym}
    end
  end
  ...
  if Rails.env == "test"
    ...
    @provider =
      FileProvider.from(File.expand_path('../../../config/entrain.conf', __FILE__)) ||
      EnvProvider.new
  end
