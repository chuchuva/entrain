...
module SiteSettingExtension
  ...
  def add_override!(name, val)
    ...
    if type == types[:enum]
      raise Entrain::InvalidParameters.new(:value) unless enum_class(name).valid_value?(val)
    end

    if v = validators[name]
      validator = v[:class].new(v[:opts])
      unless validator.valid_value?(val)
        raise Entrain::InvalidParameters.new(validator.error_message)
      end
    end
  ...
  def set(name, value)
    if has_setting?(name) && is_valid_data?(name, value)
      ...
      Entrain.request_refresh! if requires_refresh?(name)
    else
